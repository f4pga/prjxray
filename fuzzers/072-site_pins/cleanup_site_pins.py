...
def main():
  ...
  site_pin_to_wires = create_site_pin_to_wire_maps(site_pins['tile_name'], site_pins['nodes'])
  ...
  for site in output_site_pins['sites']:
    ...
    x_coord = int(coordinate.group(2))
    y_coord = int(coordinate.group(3))
    site['name'] = 'X{}Y{}'.format(x_coord - min_x_coord, y_coord - min_y_coord)
    ...
    for site_pin in site['site_pins']:
      ...
      if site_pin['name'] in site_pin_to_wires:
        ...
        print(('***WARNING***: Site pin {} for tile type {} is not connected, '
              'make sure all instaces of this tile type has this site_pin '
               'disconnected.').format(
                   site_pin['name'], site_pins['tile_type']), file=sys.stderr)
