...
if sys.platform == "win32":
    ...
    class Console:
        ...
        def unconfigure(self):
            termios.tcsetattr(
                self.fd, termios.TCSAFLUSH, self.default_settings)
...
class LiteXTerm:
    def __init__(
            self, serial_boot, kernel_image, kernel_address, json_images,
            no_crc):
        ...
        if json_images is not None:
            ...
            self.boot_address = self.mem_regions[list(
                self.mem_regions.keys())[-1]]
            f.close()
        self.no_crc = no_crc
        ...
        while retry:
            ...
            if not self.no_crc:
                ...
                if reply == sfl_ack_success:
                    ...
                    print(
                        "[LXTERM] Got unknown reply '{}' from the device, aborting."
                        .format(reply))
                    return 0
            else:
                retry = 0
    ...
    def upload(self, filename, address):
        ...
        f.seek(0, 0)
        print(
            "[LXTERM] Uploading {} to 0x{:08x} ({} bytes)...".format(
        ...
        f.close()
        print(
            "[LXTERM] Upload complete ({0:.1f}KB/s).".format(
                length / (elapsed * 1024)))
        return length
...
def _get_args():
    ...
    parser.add_argument(
        "--kernel-adr", default="0x40000000", help="kernel address")
    parser.add_argument(
        "--images",
        default=None,
        help="json description of the images to load to memory")
    parser.add_argument(
        "--no-crc",
        default=False,
        action='store_true',
        help="disable CRC check (speedup serialboot)")
...
def main():
    args = _get_args()
    term = LiteXTerm(
        args.serial_boot, args.kernel, args.kernel_adr, args.images,
        args.no_crc)
