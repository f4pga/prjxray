...
class ConfigurationPacketizer {
       public:
	class iterator
	    : std::iterator<std::input_iterator_tag, ConfigurationPacket<typename ArchType::ConfRegType>> {
	       public:
		iterator(const typename ArchType::Part* part,
		...
		const ConfigurationPacket<typename ArchType::ConfRegType>& operator*() const;
		const ConfigurationPacket<typename ArchType::ConfRegType>* operator->() const;
		...
		State state_;
		typename Configuration<ArchType>::FrameMap::const_iterator frame_cur_;
		typename Configuration<ArchType>::FrameMap::const_iterator frame_end_;
		absl::optional<uint32_t> frame_address_;
		absl::optional<ConfigurationPacket<typename ArchType::ConfRegType>> packet_;
