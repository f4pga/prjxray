...
def bit_to_bits(bitread, part_yaml, bit_file, bits_file, frame_range=None):
    ...
    subprocess.check_output('{} --part_file {} {} -o {} -z -y {}'.format(
        bitread, part_yaml, frame_range_arg, bits_file, bit_file), shell=True)
...
def main():
    ...
    parser.add_argument('--part', help="Name of part being targetted.", **part_kwargs)
    parser.add_argument('--bitread', help="Name of part being targetted.", default=default_bitread)
    parser.add_argument('--frame_range', help="Frame range to use with bitread.")
    ...
    with tempfile.NamedTemporaryFile() as bits_file:
        ...
        bits_to_fasm(args.db_root, bits_file.name, args.verbose, args.canonical)
